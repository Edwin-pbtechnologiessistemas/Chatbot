...
class ChatRAG_Database {
    ...
    public function searchProducts($query, $limit = 5) {
        $sql = $this->wpdb->prepare(
            "SELECT * FROM {$this->tables['products']} 
             WHERE MATCH(product_name, short_description, specifications, keywords) 
             ...
             ORDER BY 
                CASE 
                    ...
                    ELSE 3
                END,
                MATCH(product_name, short_description, specifications, keywords) AGAINST (%s) DESC
             LIMIT %d",
            $query,
    ...
    // Método para obtener el objeto wpdb si es necesario (con precaución)
